...
#### **actionClicked**
Triggers when the user clicks a button of a message with type `action`. This type of message can only be generated by the BFF package. The `actionClicked` output event requires a defined action and pauses the workings of the chatbot widget, waiting for a method call (`completeAction`) that has the same defined action. It allows the developer to handle some things outside the chatbot (e.g. a payment) before returning to the chatbot conversation.
...
2. When the user clicks this button, the defined `actionClicked` output event will fire and the chatbot is disabled.
